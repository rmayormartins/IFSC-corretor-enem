/* ============================================
   CORRETOR ENEM — APP.JS v2
   Fix: timeout nas chamadas, diagnóstico de erro
   ============================================ */

const STORAGE_KEY = 'corretor-enem-config';
const CALL_TIMEOUT_MS = 90000; // 90s máximo por LLM
...
function openConfig() { $('#worker-url').value = config.workerUrl; modalConfig.hidden = false; }
function closeConfig() { modalConfig.hidden = true; }
function saveConfig() {
  const url = $('#worker-url').value.trim().replace(/\/$/, '');
  if (url && !url.startsWith('https://')) { alert('A URL deve começar com https://'); return; }
...
async function checkEndpoint() {
  ...
  try {
    ...
    if (res.ok) {
      ...
      const nKeys = Object.values(keys).filter(Boolean).length;
      statusText.textContent = `Conectado · ${nKeys}/4 chaves ativas`;
      console.log('[Corretor ENEM] Health check:', data);
    } else throw new Error('HTTP ' + res.status);
  } catch (err) {
    statusDot.classList.add('err');
    statusText.textContent = 'Endpoint inacessível — ' + err.message;
    console.error('[Corretor ENEM] Health check failed:', err);
  }
}
...
async function corrigir() {
  ...
  if (!config.workerUrl) { alert('Configure a URL do Worker primeiro.'); openConfig(); return; }
  ...
  for (const p of Object.keys(PROVIDERS)) {
    if ($(`#sel-${p}`).checked) {
      selected.push({ provider: p, model: $(`#model-${p}`).value });
    }
  }
  ...
  try {
    if (parallel) {
      await Promise.allSettled(selected.map(async sel => {
        ...
        console.log(`[${sel.provider}]`, r.error ? 'ERRO: ' + r.error : 'OK');
      }));
    } else {
    ...
    // Verifica se TODAS falharam — mostra alerta útil
    const allErrors = selected.every(s => results[s.provider].error);
    if (allErrors) {
      const errMsgs = selected.map(s => `• ${PROVIDERS[s.provider].label}: ${results[s.provider].error}`).join('\n');
...
// Wrapper com timeout — nunca deixa o fetch pendurar
async function callProviderSafe(sel, prompt, deterministic) {
  ...
  try {
    ...
    try {
      ...
      return { error: 'Resposta da LLM não é JSON válido: ' + (data.text || '').slice(0, 120), elapsed, model: sel.model };
    }
    ...
    // Erros de CORS, rede, DNS aparecem aqui como TypeError
    return { error: err.message + ' (verifique CORS/Worker)', elapsed, model: sel.model };
  }
}
    ...
    if (r.error) {
      return `
        <div class="summary-card" data-provider="${sel.provider}">
          <div class="summary-head"><strong>${p.label}</strong><span class="summary-model">${sel.model}</span></div>
...
function renderCompMatrix(results, selected) {
  ...
  if (validResults.length === 0) { $('#comp-matrix').innerHTML = '<p style="padding:24px;text-align:center;color:var(--warn)">Nenhuma LLM respondeu com sucesso.</p>'; return; }
  const comps = ['c1', 'c2', 'c3', 'c4', 'c5'];
  const header = `<tr><th>Competência</th>${validResults.map(s => `<th style="text-align:center">${PROVIDERS[s.provider].label}</th>`).join('')}<th style="text-align:center">Divergência</th></tr>`;
  const rows = comps.map(cid => {
    const notas = validResults.map(s => { const c = results[s.provider].parsed.competencias[cid]; return c ? c.nota : 0; });
...
function renderDetails(results, selected) {
  ...
  grid.innerHTML = selected.filter(s => !results[s.provider].error).map(sel => {
    ...
    return `<div class="detail-card"><div class="detail-head"><span class="dot dot-${sel.provider === 'openrouter' ? 'os' : sel.provider}"></span><strong>${p.label}</strong><span class="muted" style="font-family:var(--mono);font-size:11px">${sel.model}</span></div>${zeramento}${comps}${obs}</div>`;
  }).join('');
}
...
updateStats();
checkEndpoint();
if (!config.workerUrl) setTimeout(openConfig, 400);
